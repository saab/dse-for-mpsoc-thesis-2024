...
public class MPSoC {
    ...
    /**
     * Add the OCM switch to the platform. Connects all cores to the switch.
     */
    public void AddOCMSwitch() {
        if (this.platform == null) {
            throw new IllegalStateException("Platform must be added first");
        }
        if (this.PSCores.isEmpty()) {
            throw new IllegalStateException("Processing system cores must be added first");
        }
        ...
        this.PSCores.forEach((core, moduleName) -> {
            ...
            sGraph.connect(
                    OCMSwitch,
                    core,
                    "OCMSwitchPort1", // any switch port, Rodolfo: doesn't affect IDeSyDe exploration
                    PORT_TO_OCMSWITCH,
                    EdgeTraits.PhysicalConnection,
                    EdgeTraits.VisualConnection);
